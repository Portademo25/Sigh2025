...
<head>
    ...
    <title>{{ $tipoDocumento }} - SIGH</title>
    <style>
...
<body>
    <div class="wrapper">
        <table class="main-table">
            <tr>
                <td class="header">
                    {{-- Usamos el mismo método de embebido que ya te funcionó --}}
                    <img src="{{ $message->embed(public_path('images/logo_fona.png')) }}" alt="Logo FONA" style="height: 80px; width: auto;">
            ...
            <tr>
                <td class="content">
                    <p>Estimado(a) <strong class="text-uppercase">{{ $nombre }}</strong>,</p>
                    <p>Le informamos que se ha generado un nuevo documento digital desde su ficha de trabajador:</p>

                    <div class="document-box">
                        <p style="margin: 0;"><strong>Documento:</strong> {{ $tipoDocumento }}</p>
                        <p style="margin: 0;"><strong>Fecha de Emisión:</strong> {{ date('d/m/Y h:i A') }}</p>
                    </div>
            ...
            <tr>
                <td class="footer">
                    ...
                    <small>Sistema Integrado de Gestión Humana (SIGH) &copy; {{ date('Y') }}</small>
                </td>
            </tr>
        </table>
    </div>
