...
<div class="container py-4">
    ...
    <div class="card border-0 shadow-sm p-3 mb-4">
        <form action="{{ route('rrhh.personal.pagos.index') }}" method="GET" class="d-flex">
            <input type="text" name="buscar" class="form-control me-2" placeholder="Buscar por cédula o nombre..." value="{{ request('buscar') }}">
    ...
    <div class="table-responsive bg-white rounded shadow-sm">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td class="ps-3 fw-bold">{{ number_format($p->cedper, 0, '', '.') }}</td>
                    <td>{{ $p->nomper }} {{ $p->apeper }}</td>
                    <td class="text-center">
                        <a href="{{ route('rrhh.personal.pagos', $p->cedper) }}" class="btn btn-info btn-sm text-white px-3 fw-bold shadow-sm">
