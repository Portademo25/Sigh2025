...
<body>
    <table class="header-table">
        <tr>
            <td width="20%"><img src="{{ public_path('images/logo_ministerio.png') }}" style="height: 55px;"></td>
    ...
    <table class="info-table">
        <tr>
            ...
            <td width="52%">{{ $user->name }} {{ $user->apellido }}</td>
   ...
   <table class="data-table" style="width: 100%; border-collapse: collapse;">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ str_pad($asig->codcon, 10, "0", STR_PAD_LEFT) }}</td>
            <td>{{ $asig->nomcon ?? 'CONCEPTO SIN NOMBRE' }}</td>
            <td class="text-right">{{ number_format($asig->valcalcur, 2, ',', '.') }}</td>
            <td></td>
        </tr>
        @empty
        {{-- Esto te ayudará a saber si la variable llegó vacía --}}
        @endforelse

        {{-- Listar Deducciones --}}
    {{-- Listar Deducciones (SSO, FAOV, PIE, PENSION con códigos P1 y P2) --}}
@foreach($deducciones as $deduc)
<tr>
    <td>{{ str_pad($deduc->codcon, 10, "0", STR_PAD_LEFT) }}</td>
    <td>{{ $deduc->nomcon }}</td>
    <td></td>
    <td style="text-align: right;">
        {{-- Mostramos el monto negativo formateado correctamente --}}
        {{ number_format($deduc->valcalcur, 2, ',', '.') }}
    </td>
</tr>
@endforeach
    </tbody>
    ...
    <td class="text-right">
        {{ number_format($asignaciones->sum('valcalcur'), 2, ',', '.') }}
    </td>
    <td class="text-right">
        {{-- Aquí sumamos lo que realmente se filtró (P1 o P2) --}}
        -{{ number_format(abs($deducciones->sum('valcalcur')), 2, ',', '.') }}
    </td>
    </tr>
        <tr class="bold">
    ...
    <div class="footer">
        Sigh Sistema de Gestion Humana | Fecha de Impresión: {{ date('d/m/Y h:i:s a') }}
    </div>
</body>
</html>
