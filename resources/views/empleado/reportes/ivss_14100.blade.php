...
<body>
    ...
    <table>
        ...
        <tr>
            <td colspan="2"><b>RAZÓN SOCIAL:</b><br>{{ $empresa['nombre'] ?? 'NOMBRE DE LA INSTITUCION' }}</td>
            <td colspan="2"><b>NÚMERO PATRONAL:</b><br>{{ $empresa['patronal'] ?? '000-00000-0' }}</td>
        </tr>
        <tr>
            <td colspan="2" rowspan="2"><b>DIRECCIÓN:</b><br>{{ $empresa['direccion'] ?? '' }}</td>
            <td><b>TELÉFONO:</b><br>{{ $empresa['telefono'] ?? '' }}</td>
            <td><b>RIF:</b><br>{{ $empresa['rif'] ?? '' }}</td>
        </tr>
        <tr>
            <td colspan="2"><b>CORREO ELECTRÓNICO:</b><br>{{ $empresa['email'] ?? '' }}</td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tr>
            <td colspan="3"><b>APELLIDOS Y NOMBRES:</b><br>{{ $trabajador->nomper }} {{ $trabajador->apeper }}</td>
            <td><b>CÉDULA DE IDENTIDAD N°:</b><br>{{ $trabajador->codper }}</td>
        </tr>
        <tr>
            <td colspan="2"><b>FECHA DE INGRESO:</b><br>{{ \Carbon\Carbon::parse($trabajador->fecingper)->format('d/m/Y') }}</td>
    ...
    <table>
        ...
        <tr class="text-center">
            <td><b>{{ $ano }}</b></td>
            @php $totalAnual = 0; @endphp
            @for ($i = 1; $i <= 12; $i++)
                @php
                    $monto = $detalles->get($i)->monto ?? 0;
                    $totalAnual += $monto;
                @endphp
                <td>{{ $monto > 0 ? number_format($monto, 2, ',', '.') : '-' }}</td>
            @endfor
            <td><b>{{ number_format($totalAnual, 2, ',', '.') }}</b></td>
        </tr>
    </table>
    ...
    <div style="margin-top: 5px; text-align: right; font-size: 8px;">
        FORMA: 14-100 | Fecha de Emisión: {{ date('d/m/Y') }}
    </div>
