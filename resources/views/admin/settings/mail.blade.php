                ...
                <div class="card-body p-4">
                    ...
                    @if($errors->has('test'))
                        <div class="alert alert-danger border-0 shadow-sm">
                            <i class="bi bi-exclamation-triangle-fill me-2"></i> {{ $errors->first('test') }}
                        </div>
                    @endif
                    ...
                    <form action="{{ route('admin.mail.update') }}" method="POST">
                        ...
                        <div class="row g-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" name="host" class="form-control" value="{{ old('host', $mail->host ?? '') }}" required>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label fw-bold small text-uppercase">Puerto</label>
                                <input type="number" name="port" class="form-control" value="{{ old('port', $mail->port ?? '') }}" required>
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="username" class="form-control" value="{{ old('username', $mail->username ?? '') }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-uppercase">Contraseña</label>
                                <input type="password" name="password" class="form-control" value="{{ old('password', $mail->password ?? '') }}" required>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-uppercase">Remitente (Email)</label>
                                <input type="email" name="from_address" class="form-control" value="{{ old('from_address', $mail->from_address ?? '') }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-uppercase">Nombre Remitente</label>
                                <input type="text" name="from_name" class="form-control" value="{{ old('from_name', $mail->from_name ?? '') }}" required>
                            </div>
                        </div>
                    ...
                    {{-- FORMULARIO APARTE PARA EL TEST --}}
                    @if($mail)
                        <form action="{{ route('admin.mail.test') }}" method="POST" style="display:inline;">
                            @csrf
